...
- (void)testErrorIsPresentedAsSheetWithDetails {
    ...
    NSError *shown = [wc valueForKey:@"presentedError"];
    XCTAssertTrue([shown.localizedDescription containsString:@"“broken.zip” could not be extracted"],
                 @"the sheet title should name the archive, got %@", shown.localizedDescription);
    XCTAssertTrue([shown.localizedRecoverySuggestion containsString:@"Unrecognized archive format"],
