...
- (void)testDirectoryReplacedBySymlinkIsNotFollowed {
    ...
    XCTAssertNil([self quarantineAtPath:outsideFile],
                @"a file reached through the swapped-in symlink should not be marked");
    XCTAssertNil([self quarantineAtPath:outside],
...
- (void)testReadOnlyFileReplacedBySymlinkKeepsTargetMode {
    ...
    XCTAssertTrue((st.st_mode & 07777) == 0444,
    ...
    XCTAssertTrue(st.st_ctimespec.tv_sec == before.st_ctimespec.tv_sec &&
